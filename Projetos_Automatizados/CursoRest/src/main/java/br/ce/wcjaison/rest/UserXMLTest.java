...
//		.body("user.filhos.name", hasItems("Luizinho", "Zezinho"))
	...
	public void deveFazerPesquisasAvancadasComXMLEJava() {
		...
		String teste = given() // Pré Condições
		.when()
		.get("/usersXML") // Ação
		.then() // Assertivas
		.statusCode(200)
		...
		ArrayList<NodeImpl> nomes = given() // Pré Condições
				.when()
				.get("/usersXML") // Ação
				.then() // Assertivas
				.statusCode(200)
	...
	public void deveFazerPesquisasAvancadasComXPath() {
		...
		//.body(hasXPath("//name[text() = 'Ana Julia']/following-sibling::filhos", allOf(containsString("Zezinho"), containsString("Luzinho"))))
